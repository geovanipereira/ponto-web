...
public class TestModelPonto {
    ...
    public void gradeTest() throws ParseException {
        ...
        grade.setId(Short.parseShort("-1"));
        grade.setDescricao("teste");

        GradeHorario horario = new GradeHorario();
        //horario.getId().setGradeHorario(grade);
        horario.getId().setNumDia((short) 2);
        horario.setLog12e((short) 1);

        //grade.getHorarios().add(horario);

        IDAO dao = DAOFactory.getInstance().getDAO(Grade.class);
        int size = dao.query("der.ponto.Grade").size();
        dao.save(grade);
        assertEquals(size+1, dao.query("der.ponto.Grade").size());
        dao.delete(grade);
        assertEquals(size, dao.query("der.ponto.Grade").size());
         * */
    }
    ...
    public void ocorrenciaTest() throws DAOException, DAOValidationException, DAOConstraintException {
        ...
        dao.save(oco);
        //assertEquals(1, dao.query("der.ponto.Ocorrencia").size());
        Ocorrencia oco2 = new Ocorrencia();
        oco2.setId(oco.getId());
        //oco2.setDataOcorrencia(oco.getDataOcorrencia());
        dao.delete(oco2);
    }
